                ...
                <div className={`block text-sm ${!inflationAdjust ? 'opacity-50' : ''}`} aria-disabled={!inflationAdjust}>
                  <div className="flex items-center gap-2">
                    <input
                      type="radio"
                      id="infl-hist-nasdaq"
                      name="inflationSourceNasdaq"
                      checked={useHistoricalInflation}
                      onChange={() => onParamChange('useHistoricalInflation', true)}
                      disabled={!inflationAdjust}
                    />
                    <label htmlFor="infl-hist-nasdaq" className={`text-sm ${!inflationAdjust ? 'text-slate-500 dark:text-slate-400' : ''}`}>Use Historical Inflation</label>
                  </div>
                  <div className="flex items-center mt-2 gap-2">
                    <input
                      type="radio"
                      id="infl-custom-nasdaq"
                      name="inflationSourceNasdaq"
                      checked={!useHistoricalInflation}
                      onChange={() => onParamChange('useHistoricalInflation', false)}
                      disabled={!inflationAdjust}
                    />
                    <label htmlFor="infl-custom-nasdaq" className={`text-sm ${!inflationAdjust ? 'text-slate-500 dark:text-slate-400' : ''}`}>Use Custom Inflation Rate</label>
                    <NumericInput
